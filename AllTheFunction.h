...
void menu(void);//it shows the menu bar
void setcolor(int);
void _SeeHistory(char);//see the history and about elements using File Handling
void Print_All_Elemnts(void);// print all the elements
void _SingleElement(void);//print element according to the  given input
void table(int);//this will print the element`s properties according to given data
...
strcpy(atom[58].symbol,"Pr");
	...
	atom[117].atomicwt=294.240;

     fflush(stdin);//clear the buffer
////////////////////now show the Element reviews//////////
    system("cls");
    gotoxy(30,12);
    setcolor(7);
    printf("Element Name : %s",atom[atomicNumber-1].name);

    gotoxy(30,14);
    setcolor(2);
    printf("Element symbol : %s",atom[atomicNumber-1].symbol);


    gotoxy(30,16);
    setcolor(3);
    printf("Element Number : %d",atom[atomicNumber-1].atomicnum);


    gotoxy(30,18);
    setcolor(4);
    printf("Element Weight : %f",atom[atomicNumber-1].atomicwt);
...
void menu(void) //it shows the menu
{
...
void Print_All_Elemnts(){

    FILE *AllDataOfElement;

    char ch;

    AllDataOfElement =fopen("allelements.txt","r+");

    SetConsoleTitle("ALL ELEMENTS :- Periodic Table");

    if(AllDataOfElement == NULL)
    {
            setcolor(3);
        gotoxy(40,12);
        printf(" ooppps !!! SomeThing Went Wrong ! Press Any Key To  Go Back");
    ...
    {
        while(!feof(AllDataOfElement))
        {
            ch =fgetc(AllDataOfElement);

            printf("%c",ch);
        }

    }
      fclose(AllDataOfElement);
        getch();
        system("cls");
        menu();
}

//print indivisual elements
...
void _SingleElement(void)
{
  ...
  fflush(stdin);
    ...
    SetConsoleTitle("Search The Elements");
readDataAgain:
  setcolor(13);
    mvLine(20,8,10,'*');
     mvLine(22,8,10,'*');
    ...
    gotoxy(20,5);
    setcolor(6);
    printf("Enter Atomic Number Elements :  ");
